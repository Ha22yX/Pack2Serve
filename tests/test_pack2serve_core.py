...
class Pack2ServeCoreTests(unittest.TestCase):
    ...
    def test_panel_service_hides_offline_player_when_same_player_is_online(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            tmp_path = Path(temp)
            server_dir = tmp_path / "workspace/servers/sample-server"
            (server_dir / "pack2serve").mkdir(parents=True)
    ...
    def test_cli_curseforge_providers_include_default_no_key_providers(self) -> None:
        ...
        self.assertEqual([provider.name for provider in providers], ["curse-tools", "curse-maven"])

    def test_default_curseforge_providers_keep_no_key_resolver_without_official_key(self) -> None:
        ...
        self.assertEqual([provider.name for provider in without_key], ["curse-tools", "curse-maven"])
